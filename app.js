(() => {
  const terms = (Array.isArray(window.VIBE_TERMS) ? [...window.VIBE_TERMS] : []).sort(
    (a, b) =>
  ...
  function isCodeLike(line) {
    ...
    return (
      /^([+/<>@#*\-]|\.|---|const |let |var |expect\(|if\s*\(|export |import |npm |process\.|\/[a-z]|`)/i.test(
        trimmed
      ) ||
      /\.(ts|tsx|js|jsx|md|env|json|css|html)\b/.test(trimmed) ||
      /=>|===|!==|console\.|status\(/.test(trimmed)
    );
  }

  function renderExampleContent(exampleText) {
    modalBody.replaceChildren();
    const lines = String(exampleText || "").split("\n");

    lines.forEach((line) => {
      if (line.trim() === "") {
        const spacer = document.createElement("div");
        spacer.className = "example-line example-line--spacer";
        spacer.innerHTML = "&nbsp;";
      ...
      const codeLike = isCodeLike(line) || isMostlyLatin(line);
      el.className = codeLike
        ? "example-line example-line--ltr"
        : "example-line example-line--rtl";
      el.dir = codeLike ? "ltr" : "rtl";
      el.textContent = line;
  ...
  function openExampleModal(termName, exampleText, trigger) {
    ...
    renderExampleContent(exampleText);
    modal.hidden = false;
  ...
  function render(list) {
    ...
    list.forEach((item, i) => {
      ...
      const meaningActions = document.createElement("div");
      meaningActions.className = "meaning-actions";
      ...
      if (item.example) {
        ...
        meaningActions.appendChild(exampleBtn);
      }

      meaningCell.appendChild(meaningActions);
